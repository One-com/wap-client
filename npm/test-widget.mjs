...
test('showAgentSelector:false hides the picker but keeps the default agent working', async () => {
    ...
    // The role the hidden <select> resolved is still what the request carries,
    // so hiding the picker does not strand the session on no agent at all.
...
test('showAgentSelector:false still completes a proactive handoff', async () => {
    ...
    // switchAgent() and maybeContinueHandoff() both resolve out of the picker's
    // options, so the element has to stay built even when it is never shown.
