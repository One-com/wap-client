...
echo "\nAgent selector layout flag\n";

reset_state();
ChatWidget::register(base_args(['layout' => ['showAgentSelector' => false]]));
check(
    'showAgentSelector:false survives sanitize_layout',
    false === (page_record('my-plugin-chat')['layout']['showAgentSelector'] ?? null)
);

reset_state();
register_and_build_menu(base_args(['layout' => ['showAgentSelector' => false]]));
do_action('admin_enqueue_scripts', 'toplevel_page_my-plugin-chat');
check(
    'showAgentSelector reaches the widget config',
    false === ($GLOBALS['wap_localized']['wap-client-chat-my-plugin-chat']['layout']->showAgentSelector ?? null)
);

reset_state();
ChatWidget::register(base_args());
check(
    'omitting showAgentSelector leaves the key unset so the JS default (shown) applies',
    !array_key_exists('showAgentSelector', page_record('my-plugin-chat')['layout'])
);

// Non-bools are cast, not dropped: a host reading a stored option gets '0'/''
// from the options table, and dropping those would silently show the picker.
reset_state();
ChatWidget::register(base_args(['layout' => ['showAgentSelector' => '0']]));
check(
    'a non-bool showAgentSelector is cast rather than dropped',
    false === (page_record('my-plugin-chat')['layout']['showAgentSelector'] ?? null)
);

reset_state();
ChatWidget::register(base_args(['layout' => ['showAgentSelector' => 1]]));
check(
    'a truthy non-bool showAgentSelector casts to true',
    true === (page_record('my-plugin-chat')['layout']['showAgentSelector'] ?? null)
);
...
// -----------------------------------------------------------------------------
// WPIN-9086 — i18n overrides (incl. consentText)
// -----------------------------------------------------------------------------

echo "\nWPIN-9086 — i18n overrides\n";

reset_state();
register_and_build_menu(base_args());
check('no i18n override → stored i18n args are empty', [] === page_record('my-plugin-chat')['i18n']);

reset_state();
register_and_build_menu(base_args(['i18n' => ['consentText' => 'Custom consent copy.']]));
check(
    "an explicit i18n key survives to the page record",
    'Custom consent copy.' === (page_record('my-plugin-chat')['i18n']['consentText'] ?? null)
);

reset_state();
register_and_build_menu(base_args(['i18n' => 'not-an-array']));
check('a non-array i18n falls back to no overrides', [] === page_record('my-plugin-chat')['i18n']);

reset_state();
register_and_build_menu(base_args(['i18n' => ['consentText' => 'Accepting mints a WordPress Application Password.']]));
do_action('admin_enqueue_scripts', 'toplevel_page_my-plugin-chat');
$i18n = $GLOBALS['wap_localized']['wap-client-chat-my-plugin-chat']['i18n'];
check(
    'the override reaches the widget config',
    'Accepting mints a WordPress Application Password.' === ($i18n['consentText'] ?? null)
);
check('unspecified keys still take the built-in default', 'Before you start' === ($i18n['consentTitle'] ?? null));
check('other built-in keys are untouched', 'You' === ($i18n['you'] ?? null));

reset_state();
add_filter('wap_client_i18n', static function (array $i18n): array {
    $i18n['consentText'] = 'Site-wide override.';
    return $i18n;
});
register_and_build_menu(base_args());
do_action('admin_enqueue_scripts', 'toplevel_page_my-plugin-chat');
check(
    'wap_client_i18n filter can override site-wide',
    'Site-wide override.' === ($GLOBALS['wap_localized']['wap-client-chat-my-plugin-chat']['i18n']['consentText'] ?? null)
);

reset_state();
add_filter('wap_client_i18n', static function (array $i18n): array {
    $i18n['consentText'] = 'Filter overrides the per-page value too.';
    return $i18n;
});
register_and_build_menu(base_args(['i18n' => ['consentText' => 'Per-page value.']]));
do_action('admin_enqueue_scripts', 'toplevel_page_my-plugin-chat');
check(
    'the wap_client_i18n filter runs after the per-page merge, so it wins last',
    'Filter overrides the per-page value too.'
        === ($GLOBALS['wap_localized']['wap-client-chat-my-plugin-chat']['i18n']['consentText'] ?? null)
);
