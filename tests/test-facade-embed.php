...
// The gate that stops the widget appearing on a site where the AJAX auth flow
// could never succeed.
reset_state();
$GLOBALS['wap_ssl'] = false;
\WapClient::register_chat_embed(embed_args());
check(
    'registration is refused when Application Passwords are unavailable',
    [] === private_prop(ChatWidget::class, 'pages')->getValue()
);
check('a refused embed reports no chatbox', !\WapClient::has_chatbox('content-ai'));
check('a refused embed returns no markup', '' === \WapClient::get_chatbox('content-ai'));
