...
// ConsentGate reads through this — plain pass-through unless a host hooks wap_client_consent_granted.
function apply_filters(string $hook, $value)
{
    return $value;
}

// Nothing to report as "still live" here — see wap_test_seed_password() below.
function wp_get_application_passwords(int $user_id): array
{
    return [];
}

// AppPasswordGuard::current_item() prefers this class; seeded directly by
// tests that need should_rotate()/is_expired() to see a real "created" timestamp.
...
// Fresh fetch is cached; the second get() serves from cache. Seed a fresh
// backing App Password so this tests GRND caching, not app-password rotation.
update_user_meta(7, 'wap_app_password_uuid_wp-rocket', 'uuid-7-fresh');
wap_test_seed_password(7, 'uuid-7-fresh', time());
...
update_user_meta(11, 'wap_client_consent_wp-rocket', time()); // provision() now refuses without consent on record.
...
// provision() doesn't populate this file's WP_Application_Passwords shim — seed "created" directly.
$uuid_11 = get_user_meta(11, 'wap_app_password_uuid_wp-rocket', true);
wap_test_seed_password(11, (string) $uuid_11, time());
...
// -----------------------------------------------------------------------------
// 4. A cached GRND is not served when the App Password should rotate, even
//    though the GRND's own exp is nowhere near due (the sync fix, RKM-2014).
// -----------------------------------------------------------------------------

echo "app password should_rotate forces a cache miss\n";

$tm->forget(12, 'wp-rocket');
update_user_meta(12, 'wap_app_password_uuid_wp-rocket', 'uuid-12-aging');
wap_test_seed_password(12, 'uuid-12-aging', time() - (51 * 60)); // past the 50-minute soft cutoff.

// GRND-expiry grounds alone would make this a clean cache hit.
$long_lived_grnd = make_grnd([], ['exp' => time() + 3600]);
$sync_provider   = new StubProvider([wrap($long_lived_grnd)]);

$tm->get(12, 'wp-rocket', $sync_provider);
check('precondition: GRND cached with an hour left on its own exp', get_transient('wap_grnd_12_wp-rocket') !== false);

$tm->get(12, 'wp-rocket', $sync_provider);
check(
    'a cache hit is refused when the App Password should rotate, despite a fresh GRND exp',
    $sync_provider->fetches === 2
);

// Conversely: once fresh again, the same cached GRND IS served — should_rotate-specific, not a forced refetch.
$tm->forget(13, 'wp-rocket');
update_user_meta(13, 'wap_app_password_uuid_wp-rocket', 'uuid-13-fresh');
wap_test_seed_password(13, 'uuid-13-fresh', time());
$fresh_pw_provider = new StubProvider([wrap(make_grnd())]);
$tm->get(13, 'wp-rocket', $fresh_pw_provider);
$tm->get(13, 'wp-rocket', $fresh_pw_provider);
check('a fresh App Password still allows a normal cache hit', $fresh_pw_provider->fetches === 1);
